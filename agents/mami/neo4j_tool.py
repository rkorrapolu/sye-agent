...
class Neo4jKnowledgeGraphTool(Tool):
  ...
  def _query_existing(self, query_data: Dict) -> str:
    ...
    if cached_nodes:
      ...
      return json.dumps(
        {"source": "cache", "nodes": cached_nodes, "count": len(cached_nodes)}
      )
    ...
    # Return results
    if nodes:
      return json.dumps({"source": "neo4j", "nodes": nodes, "count": len(nodes)})
    else:
      return json.dumps({"source": "neo4j", "nodes": [], "count": 0})
