from smolagents import CodeAgent, InferenceClientModel, ActionStep, PromptTemplates, PlanningPromptTemplate, ManagedAgentPromptTemplate, FinalAnswerPromptTemplate
import json
...
def get_retry_feedback():
    ...
    # Remove the last two empty lines
    feedback = "\n".join(lines[:-2])
    return feedback.strip() if feedback.strip() else "Please try again with better extraction."
...
def interrupt_on_existing_node_review(memory_step, agent):
    ...
    try:
        ...
        if not existing_nodes:
            ...
            if proceed == 'y':
                ...
                memory_step.observations = (
                    "User confirmed: No existing nodes found. Proceed with generating a new knowledge graph."
                )
        ...
        if choice == 1:  # Confirm as-is
            ...
            modified_nodes = get_modified_existing_nodes(output_dict.get("existing_nodes", []))
    ...
    try:
        ...
        if choice == 1:  # Approve
            ...
            try:
                ...
                print(f"🔗 View in Neo4j Browser: http://localhost:7474/browser/")
...
''',
    ...
    print("The agent will extract symptoms, errors, and actions to build a knowledge graph.")
    print("\nExamples:")
